const Board = () => {
    ...
    return (
      <div className="game-board">
          <h1 id='turn' 
          className={(player==1)?'fontOrange':(player==0)?'fontBlack':''}>
              {nextPlayer}
          </h1>
        ...
        <div id="info">
          ...
          */}
          ...
          <h1 className={(checkForWinner(gameState)=='Player O ') ? 
          'fontBlack':(checkForWinner(gameState)=='Player X ')?'fontOrange':''}>{status}</h1>
         <div>
  ...
  const Square = ({ takeTurn, id }) => {
    ...
    return (
      <button
      className={
          (mark[tik]=='X')?'orange':(mark[tik]=='O')?'black':''
      }
